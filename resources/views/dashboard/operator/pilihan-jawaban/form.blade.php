    ...
    <div class="p-2 sm:p-5 sm:py-0 md:pt-5 space-y-5">
      ...
      <div class="flex flex-col gap-y-5 max-w-3xl mx-auto">
        ...
        <div class="flex gap-x-3">
          <div class="grow">
            <h1 class="font-semibold text-xl text-gray-800 dark:text-neutral-200">
                {{ ($mode == 'create') ? 'Tambah' : 'Update' }} pilihan jawaban
            </h1>

            <p class="text-sm text-gray-500 dark:text-neutral-500">
               {{ ($mode == 'create') ? 'Membuat' : 'Memperbarui' }} pilihan jawaban ke dalam butir soal
            </p>
          </div>
        </div>
        <!-- End Header -->
        ...
        <div class="bg-white border border-gray-200 shadow-2xs rounded-xl dark:bg-neutral-800 dark:border-neutral-700">
          <form action="{{ ($mode == 'create') ? route('operator.soal.pj.create.action', [$idps, $ids]) : route('operator.soal.pj.update.action', [$idps, $ids, $data['id_pilihan_jawaban']]) }}" method="post">
            @csrf
            @if ($mode == 'update')
            @method('PUT')
            @endif
            <div class="py-2 sm:py-4 px-2">
              <div class="p-4 space-y-5">
                <!-- WYSIWYG Editor -->
                <div id="editor">
                  {!! ($mode == 'create') ? (old('teks_jawaban') ? old('teks_jawaban') : '') : ($data['teks_jawaban']) !!}
                </div>
                ...
                <input type="hidden" name="teks_jawaban" id="contentEditor" value="{{ old('teks_jawaban') }}">
                <!-- End WYSIWYG Editor -->
                ...
                <div class="pt-5 grid sm:grid-cols-12 gap-y-1.5 sm:gap-y-0 sm:gap-x-5">
                  ...
                  <div class="sm:col-span-9">
                    <select data-hs-select='{
                      "placeholder": "Pilih kondisi jawaban...",
                    ...
                    }' name="benar" class="hidden">
                      <option value="">Choose</option>
                      <option value="1" {{ ((($mode == 'create' ? old('teks_jawaban') : $data['benar']) == '1') ? 'selected=""' : '') }} data-hs-select-option='{
                          "description": "Peserta mendapatkan poin nilai saat memilih.",
                      ...
                      <option value="0" {{ ((($mode == 'create' ? old('teks_jawaban') : $data['benar']) == '0') ? 'selected=""' : '') }} data-hs-select-option='{
                          "description": "Peserta tidak mendapatkan poin nilai saat memilih.",
                          "icon": "<svg class=\"shrink-0 size-4 text-gray-800 dark:text-neutral-200 \" xmlns=\"http://www.w3.org/2000/svg\" width=\"24\" height=\"24\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\" class=\"lucide lucide-circle-icon lucide-circle\"><circle cx=\"12\" cy=\"12\" r=\"10\"/></svg>"
